// Auto-generated by scripts/sync-pages-changelog.js. Do not edit directly.
window.CLOUDSSH_CHANGELOG = {
  "releases": [
    {
      "version": "1.6.2",
      "date": "2026-08-05",
      ...
      "sections": [
        {
          "type": "Improved",
          "items": [
            "优化 AI Agent 超时机制，大幅提升复杂部署任务的执行能力：；看门狗超时从 60 秒增加到 300 秒（5 分钟），命令执行超时从 60 秒增加到 180 秒（3 分钟）。；基础迭代次数从 30 次增加到 50 次，扩展机会从 3 次增加到 5 次，最大总迭代次数从 90 次增加到 175 次。；循环检测窗口从 5 增加到 7，重复率阈值从 60% 放宽到 70%，命令多样性阈值从 30% 放宽到 20%。；消息裁剪阈值从 40 条增加到 60 条，保留更多对话上下文。；长时间命令（>60 秒）自动每 60 秒重置看门狗计时器，防止执行期间意外超时。；前端新增进度扩展提示，自动延期时显示当前进度和延期原因。"
          ]
        }
      ]
    },
      ...
      "sections": [
        {
          "type": "Fixed",
          "items": [
            "修复多分辨率下前端页面内容被裁切与溢出的系统性问题，全面优化了响应式布局：；修复了小视口（如小屏或横屏）下登录表单被裁切且无法滚动的问题。；修复了在窄屏下，终端页面中 AI Agent 面板和 SFTP 面板使用固定宽度导致的溢出或极度挤压终端区域的 Bug。；优化了 SFTP 右键菜单的边界检测逻辑，防止在屏幕边缘展开时被视口遮挡。；为终端工具栏、底部状态栏、用户空间顶部导航等区域增补了 flex-wrap 及截断策略，彻底解决了各元素重叠或溢出的情况。"
          ]
        }
      ]
    },
      ...
      "sections": [
        {
          ...
          "items": [
            ...
            "高兼容性算法支持：；密钥交换：curve25519-sha256、ecdh-sha2-nistp256。；数据加密：aes256-gcm、aes128-gcm、aes256-ctr 等。；完整性校验：hmac-sha2-256、hmac-sha2-512。；认证机制：支持密码认证及 Ed25519 纯文本私钥认证。",
            ...
            "两层安全防线：；主观/客观拦截（Blocked Patterns）：硬编码直接拒绝高危指令（如 rm -rf /、fork 炸弹等）。；确认提醒机制（Confirmation Patterns）：对高风险操作（包管理器 apt/yum 安装卸载、服务启停、sudo 权限等）强制触发前端交互弹窗确认，用户授权后方可执行。",
            "防冬眠与看门狗重置：；在大模型调用及工具执行成功时自动重置 60 秒的看门狗超时定时器，在安全确认等待期间自动挂起超时计数。；核心执行循环（runLoop）添加 5 秒/次的活跃心跳检测，防止 Durable Object 因闲置而被 CF 平台强行 Hibernate（冬眠）断开连接。",
