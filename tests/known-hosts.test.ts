...
describe('known_hosts 浏览器边界', () => {
  ...
  it('接受签名验证后的首次指纹和完整跳板路由身份', () => {
    expect(normalizeVerifiedHostKeyMessage({
  ...
  it('仅接受格式完整的指纹变更消息', () => {
    expect(normalizeChangedHostKeyMessage({
      fingerprint: NEW_FINGERPRINT,
  ...
  it('已保存服务器必须成功写入云端后才更新本地记录', async () => {
    localStorage.setItem('cloudssh_known_hosts', JSON.stringify({
      'ssh.example.com:22': OLD_FINGERPRINT,
    }));
    vi.stubGlobal('fetch', vi.fn(async () => new Response(null, { status: 503 })));

    await expect(saveKnownFingerprint(
      'ssh.example.com',
      22,
      NEW_FINGERPRINT,
      true,
    ...
    expect(JSON.parse(localStorage.getItem('cloudssh_known_hosts')!))
      .toEqual({ 'ssh.example.com:22': OLD_FINGERPRINT });
  });
  ...
  it('已保存服务器云端更新成功后同步本地缓存', async () => {
    ...
    expect(JSON.parse(localStorage.getItem('cloudssh_known_hosts')!))
      .toEqual({ 'ssh.example.com:22': NEW_FINGERPRINT });
  });
  ...
  it('匿名连接在本地和云端均不可写时报告持久化失败', async () => {
    ...
    await expect(saveKnownFingerprint('ssh.example.com', 22, NEW_FINGERPRINT))
      .rejects.toThrow('Known-host persistence unavailable');
  });
});
