...
async function showChallenge(
  ...
  await page.evaluate(async ({ prompts: challengePrompts, canUseStoredPassword: canUseStored }) => {
...
test('renders a Serv00 password challenge and acknowledges it only after display', async ({ page }) => {
  ...
  await expect(dialog.locator('.auth-challenge-dialog__target'))
    .toContainText('s15.serv00.com:22');
  ...
  expect(await page.evaluate(() => (window as any).__authChallengeState.shown))
    .toEqual(['serv00-round-1']);
...
test('keeps a multi-prompt password and TOTP challenge usable in a mobile viewport', async ({ page }) => {
